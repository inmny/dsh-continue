...
function latestTurnBoundaryEvent(events, seedLength = 0) {
...
function hasPendingMessages(events, agent, seedLength = 0) {
    if (agent !== undefined)
        return agent.inbox.hasPending;
...
export function continueStatusFromEvents(events, agent, seedLength = 0) {
...
async function inspectSubagentStatus(ctx, sessionId, state, signal) {
    ...
    for (let order = 0; order < entries.length; order += 1) {
        ...
        if (candidateState === undefined)
            continue;
        const candidateStatus = continueStatusFromEvents(candidateState.events, candidateState.agent, candidateState.seedLength);
        const candidateBoundary = latestTurnBoundary(candidateState.events, candidateState.seedLength);
...
function expectedBoundaryEvent(events, seedLength, expected, deferredTurns = 0) {
...
async function resumeSubagent(ctx, runtime, sessionId, advertised, request, signal) {
    ...
    if (child !== undefined) {
        ...
        const liveStatus = continueStatusFromEvents(child.session.events, child, typeof child.session.header.seedLength === "number" ? child.session.header.seedLength : 0);
    ...
    try {
        await awaitWithSignal(ctx.subagents.followup(parent, sessionId, [], {
            source: {
                kind: "user",
                rpcId: wakeSourceId,
            },
...
export function createContinueRpcHandler(ctx) {
    ...
    const handle = async (endpoint, payload, signal) => {
        ...
        try {
            ...
            const current = continueStatusFromEvents(liveAgent.session.events, liveAgent, typeof liveAgent.session.header.seedLength === "number" ? liveAgent.session.header.seedLength : 0);
    ...
    return async (endpoint, payload, signal) => {
        ...
        try {
            ...
            try {
                return await awaitWithSignal(handle(endpoint, payload, operationSignal), operationSignal);
            }
            catch (error) {
...
export function apply(ctx) {
    ...
    if (subagents !== undefined) {
        ctx.effect(() => subagents.registerContinuableSetup((childCtx) => {
            if (runtime.shuttingDown) {
    ...
    ctx.effect(() => {
        const disposeRpc = ctx.connection.rpc.handle(RPC_CHANNEL, createContinueRpcHandler(ctx), { authority: "loopback" });
        return async () => {
            beginShutdown(runtime);
            await disposeRpc();
            await Promise.allSettled([...runtime.activeRequests]);
